...
func MigrateUp(db *sql.DB, migrationsDir fs.FS, migrationsRelPath string) error {
	if err := createSchemaVersionTable(db); err != nil {
		return fmt.Errorf("could not create schema_version table: %w", err)
	}

	files, err := fs.ReadDir(migrationsDir, migrationsRelPath)
	if err != nil {
	...
	for _, f := range files {
		...
		content, err := migrations.FS.ReadFile(f.Name())
		if err != nil {
