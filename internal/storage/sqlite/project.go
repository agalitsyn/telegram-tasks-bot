...
func (s *ProjectStorage) CreateProject(ctx context.Context, project *model.Project) error {
	query := `
		INSERT INTO projects (tg_chat_id, title, archived)
		VALUES (?, ?, ?)
	`
	result, err := s.db.ExecContext(ctx, query, project.TgChatID, project.Title, project.Archived)
	if err != nil {
		return err
	}
...
func (s *ProjectStorage) FetchProjectByChatID(ctx context.Context, tgChatID int64) (*model.Project, error) {
	...
	`
	var project model.Project
	err := s.db.QueryRowContext(ctx, query, tgChatID).Scan(
		&project.ID,
		&project.TgChatID,
		&project.Title,
		&project.Archived,
...
func (s *ProjectStorage) UpdateProject(ctx context.Context, project *model.Project) error {
	query := `
		...
		WHERE id = ?
	`
	_, err := s.db.ExecContext(ctx, query, project.Title, project.Archived, project.ID)
	return err
}
